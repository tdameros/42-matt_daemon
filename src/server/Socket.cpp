...
int Socket::disconnect() const { return close(_fd); }

int Socket::getFD() const { return _fd; }

unsigned short Socket::getPort() const { return _port; }

std::string Socket::getIPAddress() const { return _IPAddress; }
...
std::string Socket::networkToStr(uint32_t s_addr) {
  ...
  s_addr = ntohl(s_addr);
  ip << ((s_addr & 0xff000000) >> 24) << '.' << ((s_addr & 0xff0000) >> 16)
     << '.' << ((s_addr & 0xff00) >> 8) << '.' << (s_addr & 0xff);
  return (ip.str());
}
